...
class SincNet(nn.Module):
    ...
    def __init__(self,options):
        ...
        for i in range(self.N_cnn_lay):
            ...
            if i==0:
                ...
                self.conv.append(nn.Conv1d(self.cnn_N_filt[i-1]+self.cnn_N_filt[i-2]+self.cnn_N_filt[i-3], self.cnn_N_filt[i], self.cnn_len_filt[i]))
            else:
                self.conv.append(nn.Conv1d(self.cnn_N_filt[i-1], self.cnn_N_filt[i], self.cnn_len_filt[i]))

            current_input=int((current_input-self.cnn_len_filt[i]+1)/self.cnn_max_pool_len[i])

        self.out_filt_len = 107
        self.out_dim=N_filt
    ...
    def forward(self, x):
        ...
        for i in range(self.N_cnn_lay):
            ...
            #Concaternation of skipping layer
            if i==3:
                x=torch.cat(prev_x,1)
            ...
            #Skipping layer
            if i==0:
                stack_layer_size = x.shape[2]
                prev_x.append(x)
            if i>0 and i<3:
                x_formatted = F.upsample(x,scale_factor=self.cnn_max_pool_len[i]**i)
                x_formatted = F.pad(x_formatted,((stack_layer_size-x_formatted.shape[2])//2,-((stack_layer_size-x_formatted.shape[2])//-2)))
                prev_x.append(x_formatted)
        return x
